...
function PostForm({post}) {
    ...
    const {register, handleSubmit, watch, set, control, getValues } = useForm({
        defaultValues: {
            title: post?.title || '',
            slug: post?.slug || '',
    ...
    const submit = async (data) => {
        if(post){
            //if post is avl then update it o?w create new post in else
            const file = data.image[0] ? appwriteService.uploadFile(data.image[0]) : null
            
            //if upload then purani image delete bhi to karo
            if(file){
